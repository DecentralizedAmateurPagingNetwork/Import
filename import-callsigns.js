...
// initial message
console.log("[INFO] DAPNET Callsign Import v" + pkg.version);
...
request(options, function(err, response, body) {
	...
	csvparser(fileContent, {comment: "#", delimiter: ","}, function(err, output) {
		...
		output.forEach((c) => {
			...
			// insert callsign into DAPNET (preparations)
			let options = {
				url: config.apiUrl + "/callsigns/" + callsignName,
				method: "PUT",
				headers: {
					"Authorization": config.authHeader
				},
			...
			// insert callsign into DAPNET (actual request)
			request(options, function(err, response, body) {
				if (err) {
					console.log("[WARN] Unable to insert callsign '" + callsignName + "': " + err);
				}
				if (response && response.statusCode !== 201) {
					console.log("[WARN] Unable to insert callsign '" + callsignName + "': " + response.statusCode);
				}
			});
		});
	});
